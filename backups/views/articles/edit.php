...
<head>
    ...
    <title>Modifier l'Article - Gestion de Pharmacie</title>
    ...
    <div id="main-content" class="transition-all duration-300 ease-in-out ml-64">
        ...
        <header class="flex items-center justify-between px-8 py-6 bg-white shadow-sm">
            ...
            <div class="flex items-center gap-4">
                <a href="/Produits" class="text-blue-600 hover:underline">Retour à la liste</a>
        ...
        <!-- Main Content -->
        <div class="px-8 py-6">
            <?php /** @var array $article, $categories, $fournisseurs */ ?>
            <div class="max-w-xl mx-auto py-10">
                <h2 class="text-xl font-bold mb-6">Modifier l'article</h2>
                <form method="POST" action="/Produits/edit/<?= $article['id'] ?>" class="bg-white rounded-xl shadow p-6 space-y-4">
                    <div>
                        <label class="block text-gray-700">Nom</label>
                        <input type="text" name="nom" value="<?= htmlspecialchars($article['nom']) ?>" required class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring focus:ring-green-200" />
                    </div>
                    <div>
                        <label class="block text-gray-700">Prix (€)</label>
                        <input type="number" name="prix" value="<?= htmlspecialchars($article['prix']) ?>" step="0.01" min="0" required class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring focus:ring-green-200" />
                    </div>
                    <div>
                        <label class="block text-gray-700">Catégorie</label>
                        <select name="categorie_id" required class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring focus:ring-green-200">
                            <option value="">Sélectionner une catégorie</option>
                            <?php foreach ($categories as $categorie): ?>
                                <option value="<?= $categorie['id'] ?>" <?= $article['categorie_id'] == $categorie['id'] ? 'selected' : '' ?>><?= htmlspecialchars($categorie['nom']) ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div>
                        <label class="block text-gray-700">Fournisseur</label>
                        <select name="fournisseur_id" required class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring focus:ring-green-200">
                            <option value="">Sélectionner un fournisseur</option>
                            <?php foreach ($fournisseurs as $fournisseur): ?>
                                <option value="<?= $fournisseur['id'] ?>" <?= $article['fournisseur_id'] == $fournisseur['id'] ? 'selected' : '' ?>><?= htmlspecialchars($fournisseur['nom']) ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div class="flex justify-end">
                        <button type="submit" class="bg-[color:var(--pharmacy-green,#3A8D2F)] text-white px-4 py-2 rounded-full font-semibold hover:bg-green-700 transition">Enregistrer</button>
                    </div>
                </form>
            </div>
        </div>
