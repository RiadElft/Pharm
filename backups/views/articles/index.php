...
<head>
    ...
    <title>Articles - Gestion de Pharmacie</title>
    ...
    <div id="main-content" class="transition-all duration-300 ease-in-out ml-64">
        ...
        <header class="flex items-center justify-between px-8 py-6 bg-white shadow-sm">
            ...
            <div class="flex items-center gap-4">
                <a href="Produits/create" class="bg-[color:var(--pharmacy-green,#3A8D2F)] text-white px-4 py-2 rounded-full font-semibold hover:bg-green-700 transition">Ajouter un article</a>
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="divide-y divide-gray-100">
                            <?php foreach ($articles as $article): ?>
                                <tr>
                                    <td class="px-4 py-2"><?= htmlspecialchars($article['nom']) ?></td>
                                    <td class="px-4 py-2">€<?= htmlspecialchars($article['prix']) ?></td>
                                    <td class="px-4 py-2"><?= htmlspecialchars($article['categorie_nom'] ?? 'N/A') ?></td>
                                    <td class="px-4 py-2"><?= htmlspecialchars($article['fournisseur_nom'] ?? 'N/A') ?></td>
                                    <td class="px-4 py-2">
                                        <a href="Produits/edit/<?= $article['id'] ?>" class="text-blue-600 hover:underline mr-2">Modifier</a>
                                        <a href="Produits/delete/<?= $article['id'] ?>" class="text-red-600 hover:underline" onclick="return confirm('Supprimer cet article ?');">Supprimer</a>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
