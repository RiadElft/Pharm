...
<head>
    ...
    <title>Modifier le Produit - Gestion de Pharmacie</title>
...
<body class="bg-gray-50 min-h-screen">
    ...
    <div id="main-content" class="transition-all duration-300 ease-in-out ml-64">
        ...
        <!-- Main Content -->
        <div class="px-8 py-6">
            <?php /** @var array $produit, $categories, $fournisseurs */ ?>
            <div class="max-w-xl mx-auto py-10">
                <h2 class="text-xl font-bold mb-6">Modifier le produit</h2>
                <form method="POST" action="/Produits/edit/<?= $produit['id'] ?>" class="bg-white rounded-xl shadow p-6 space-y-4">
                    <div>
                        <label class="block text-gray-700">Nom</label>
                        <input type="text" name="nom" value="<?= htmlspecialchars($produit['nom']) ?>" required class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring focus:ring-green-200" />
                    </div>
                    <div>
                        <label class="block text-gray-700">Prix (DA)</label>
                        <input type="number" name="prix" step="1" min="0" required value="<?= htmlspecialchars($produit['prix']) ?>" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring focus:ring-green-200" />
                    </div>
                    <div>
                        ...
                        <select name="categorie_id" required class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring focus:ring-green-200">
                            ...
                            <?php foreach ($categories as $categorie): ?>
                                <option value="<?= $categorie['id'] ?>" <?= $produit['categorie_id'] == $categorie['id'] ? 'selected' : '' ?>><?= htmlspecialchars($categorie['nom']) ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div>
                        ...
                        <select name="fournisseur_id" required class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring focus:ring-green-200">
                            ...
                            <?php foreach ($fournisseurs as $fournisseur): ?>
                                <option value="<?= $fournisseur['id'] ?>" <?= $produit['fournisseur_id'] == $fournisseur['id'] ? 'selected' : '' ?>><?= htmlspecialchars($fournisseur['nom']) ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
