...
<head>
    ...
    <title>Gestion des Stocks - Gestion de Pharmacie</title>
                        ...
                        <tbody class="divide-y divide-gray-100">
                            <?php if (!empty($stockEntries)): ?>
                                <?php foreach ($stockEntries as $entry): ?>
                                    <tr>
                                        <td class="px-4 py-2 font-semibold text-gray-800"><?= htmlspecialchars($entry['produit_nom'] ?? 'N/A') ?></td>
                                        <td class="px-4 py-2 <?= ((int)($entry['quantite'] ?? 0) < 10) ? 'text-red-600 font-semibold' : 'text-gray-700' ?>">
                                            <?= htmlspecialchars($entry['quantite'] ?? '0') ?>
                                            <?php if ((int)($entry['quantite'] ?? 0) < 10): ?>
                                                <span class="ml-2 inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-red-100 text-red-800">Stock Faible</span>
                                            <?php endif; ?>
                                        </td>
                                        <td class="px-4 py-2 text-gray-700"><?= htmlspecialchars($entry['notes'] ?? '') ?></td>
                                        <td class="px-4 py-2">
                                            <a href="/stock/edit/<?= $entry['produit_id'] ?>" class="text-blue-600 hover:underline">Modifier</a>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
                            <?php else: ?>
                                <tr>
                                    <td colspan="4" class="px-4 py-6 text-center text-gray-500">
                                        Aucun stock trouvé. Veuillez ajouter des produits dans l'inventaire.
                                    </td>
                                </tr>
                            <?php endif; ?>
                        </tbody>
