...
<head>
    ...
    <title>Modifier le Stock - Gestion de Pharmacie</title>
    ...
    <div id="sidebar" class="fixed top-0 left-0 h-full w-64 bg-white shadow-lg z-30 flex flex-col transition-transform duration-300 ease-in-out">
        ...
        <nav class="flex-1 px-4 py-6">
            <ul class="space-y-2">
                ...
                <li><a href="/stock" class="flex items-center gap-3 px-4 py-2 rounded-lg bg-[color:var(--pharmacy-green)]/10 text-[color:var(--pharmacy-green)] font-medium"><svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M3 7h18M3 12h18M3 17h18"/></svg>Stock</a></li>
    ...
    <div id="main-content" class="transition-all duration-300 ease-in-out ml-64">
        ...
        <header class="flex items-center justify-between px-8 py-6 bg-white shadow-sm">
            ...
            <div class="flex items-center gap-4">
                <a href="/stock" class="text-blue-600 hover:underline">Retour à la liste</a>
        ...
        <div class="px-8 py-6">
            <?php if (!empty($_SESSION['success'])): ?>
                <div class="mb-4 p-4 rounded bg-green-100 text-green-800 font-semibold"><?= $_SESSION['success']; unset($_SESSION['success']); ?></div>
            <?php endif; ?>
            <?php if (!empty($_SESSION['error'])): ?>
                <div class="mb-4 p-4 rounded bg-red-100 text-red-800 font-semibold"><?= $_SESSION['error']; unset($_SESSION['error']); ?></div>
            <?php endif; ?>
            <?php /** @var array $stockEntry */ ?>
            <?php /** @var array $produit */ ?>
            <div class="max-w-5xl mx-auto py-10">
                <div class="flex justify-between items-center mb-6">
                    <h1 class="text-2xl font-bold text-gray-800">Modifier le Stock - <?= htmlspecialchars($produit['nom'] ?? 'Produit') ?></h1>
                </div>
                <div class="bg-white rounded-xl shadow p-6">
                    <form action="/stock/update/<?= htmlspecialchars($produit['id'] ?? 0) ?>" method="POST" class="space-y-6">
                        <div>
                            <label for="quantite" class="block text-sm font-medium text-gray-700">Quantité en stock</label>
                            <input type="number" name="quantite" id="quantite" value="<?= htmlspecialchars($stockEntry['quantite'] ?? 0) ?>" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-[color:var(--pharmacy-green)] focus:ring-[color:var(--pharmacy-green)] sm:text-sm">
                        </div>
                        <div>
                            <label for="notes" class="block text-sm font-medium text-gray-700">Notes</label>
                            <textarea name="notes" id="notes" rows="3" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-[color:var(--pharmacy-green)] focus:ring-[color:var(--pharmacy-green)] sm:text-sm"><?= htmlspecialchars($stockEntry['notes'] ?? '') ?></textarea>
                        </div>
                        <div class="pt-4">
                            <button type="submit" class="inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-[color:var(--pharmacy-green)] hover:bg-[color:var(--pharmacy-green)]/90 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[color:var(--pharmacy-green)]">
                                Enregistrer
                            </button>
                            <a href="/stock" class="inline-flex justify-center ml-3 py-2 px-4 border border-gray-300 shadow-sm text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[color:var(--pharmacy-green)]">
                                Annuler
                            </a>
                        </div>
                    </form>
