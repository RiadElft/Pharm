...
<head>
    ...
    <title>Modifier un Utilisateur - Gestion de Pharmacie</title>
    ...
    <div id="sidebar" class="fixed top-0 left-0 h-full w-64 bg-white shadow-lg z-30 flex flex-col transition-transform duration-300 ease-in-out">
        ...
        <nav class="flex-1 px-4 py-6">
            <ul class="space-y-2">
                ...
                <li><a href="/utilisateurs" class="flex items-center gap-3 px-4 py-2 rounded-lg bg-[color:var(--pharmacy-green)]/10 text-[color:var(--pharmacy-green)] font-medium"><svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z"/></svg>Utilisateurs</a></li>
    ...
    <div id="main-content" class="transition-all duration-300 ease-in-out ml-64">
        ...
        <header class="flex items-center justify-between px-8 py-6 bg-white shadow-sm">
            ...
            <div class="flex items-center gap-4">
                <a href="/utilisateurs" class="bg-gray-100 text-gray-700 px-4 py-2 rounded-full font-semibold hover:bg-gray-200 transition">Retour à la liste</a>
        ...
        <!-- Main Content -->
        <div class="px-8 py-6">
            <?php /** @var array $user, $roles */ ?>
            <div class="max-w-2xl mx-auto py-10">
                <div class="flex items-center mb-6">
                    <a href="/utilisateurs" class="mr-2 text-gray-500 hover:text-[color:var(--pharmacy-green)]">
                ...
                <div class="bg-white rounded-xl shadow p-6">
                    <form action="/utilisateurs/update/<?= $user['id'] ?>" method="post" class="space-y-4">
                        <div>
                            <label for="nom" class="block text-sm font-medium text-gray-700 mb-1">Nom</label>
                            <input type="text" id="nom" name="nom" value="<?= htmlspecialchars($user['nom']) ?>" required class="w-full rounded-lg border-gray-300 border p-2 focus:ring-2 focus:ring-[color:var(--pharmacy-green)] focus:border-transparent">
                        </div>
                        <div>
                            <label for="email" class="block text-sm font-medium text-gray-700 mb-1">Email</label>
                            <input type="email" id="email" name="email" value="<?= htmlspecialchars($user['email']) ?>" required class="w-full rounded-lg border-gray-300 border p-2 focus:ring-2 focus:ring-[color:var(--pharmacy-green)] focus:border-transparent">
                        </div>
                        <div>
                            <label for="password" class="block text-sm font-medium text-gray-700 mb-1">Nouveau mot de passe (laisser vide pour ne pas modifier)</label>
                            <input type="password" id="password" name="password" class="w-full rounded-lg border-gray-300 border p-2 focus:ring-2 focus:ring-[color:var(--pharmacy-green)] focus:border-transparent">
                        </div>
                        <div>
                            <label for="role_id" class="block text-sm font-medium text-gray-700 mb-1">Rôle</label>
                            <select id="role_id" name="role_id" required class="w-full rounded-lg border-gray-300 border p-2 focus:ring-2 focus:ring-[color:var(--pharmacy-green)] focus:border-transparent">
                                <?php foreach ($roles as $role): ?>
                                    <option value="<?= $role['id'] ?>" <?= $user['role_id'] == $role['id'] ? 'selected' : '' ?>><?= htmlspecialchars($role['nom']) ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        <div class="flex justify-end pt-4">
                            <a href="/utilisateurs" class="bg-gray-100 text-gray-700 px-4 py-2 rounded-full font-medium hover:bg-gray-200 transition mr-2">Annuler</a>
